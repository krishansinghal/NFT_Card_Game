...
contract AVAXGods is ERC1155, Ownable(msg.sender), ERC1155Supply {
  ...
  uint256 override public totalSupply; // Total number of tokens minted
  ...
  struct GameToken {
    string name; //battle card name; set by player
    uint256 id; //battle card token id; will be randomly generated
    uint256 attackStrength; //battle card attack; generated randomly
    uint256 defenseStrength; //battle card defense; generated randomly
  }

  // Player struct to store player info
  struct Player {
    address playerAddress; //player wallet address
    string playerName; // player name; set by player during registration
    uint256 playerMana; //player mana; affected by battle results
    uint256 playerHealth; //player health; affected by battle results
    bool inBattle; // boolean to indicate if a player is in battle
  }

  // Battle struct to store battle info
  struct Battle {
    BattleStatus battleStatus; // enum to indicate battle status
    bytes32 battleHash; // a hash of the battle name
    string name; //battle name; set by player who creates battle
    address[2] players; //address array representing players in this battle
    uint8[2] moves; // uint array representing players' move
  ...
  //Initializes the contract by setting a `metadataURI` to the token collection
  //baseURI where token metadata is stored
  ...
  function _createGameToken(string memory _name) external returns (GameToken memory) {
  ...
  //Player joins battle
  //battle name; name of battle player wants to join
  ...
  //winner address
  // battle; taken from attackOrDefend function
